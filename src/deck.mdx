...
import badExampleQuery from './assets/images/bad-example-query.png';
import mutationExample from './assets/images/mutation-example.png';
...
#### The response data objects are normalized before being saved to the cache store, but this process has some steps:

<Steps>

- The response data objects are splitted 
- Each object receive an unique identifier, which is calculated by the `id` field and `__typename`
- The normalized items are stored in a flat object 
- After executing another request, the response data goes throught the same process and if there's a object with the same ID on the store, the fields of those object will be merged.
...
### Let's see the normalization steps with more details with the images provided on https://www.apollographql.com/blog/demystifying-cache-normalization/
...
<img src="https://wp.apollographql.com/wp-content/uploads/2020/07/GetAllTodos_1-1024x311.png" style={{ width: "80%" }} />

<img src="https://wp.apollographql.com/wp-content/uploads/2020/07/Frame_25-1-1024x275.png" style={{ width: "80%" }} />
...
<img src="https://wp.apollographql.com/wp-content/uploads/2020/07/Frame_27_1-1024x346.png" style={{ width: "85%" }} />
...
The normalization process is really important in order to avoid redudant data on the 
cache and manage changes to entities. Therefore, there are types of data that shouldn't be 
normalized, in cases it has large quantities and never receive updates. 

The Type Policy API let's us disable the normalization for particular types: 

<img src={disablingNormalizationExample} style={{ height: "50%" }} />
